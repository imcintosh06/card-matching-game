...
class SantasMatchingGame {
  ...
  hideCards() {
    this.cardsArray.forEach((card) => {
      card.classList.remove("visible", "flipped", "matched");
    });
  }
  ...
  flipCard(card) {
    if (this.canFlipCard(card)) {
      ...
      this.totalClicks++;
      this.ticker.innerText = this.totalClicks;
      card.classList.add("visible", "flipped");  // add the 'flipped' class
  ...
  cardMisMatch(card1, card2) {
    this.busy = true;
    setTimeout(() => {
      card1.classList.remove("visible", "flipped");
      card2.classList.remove("visible", "flipped");
      this.busy = false;
    }, 800);
  }
